...
| `one_off_date` | *(blank)* | `YYYY-MM-DD`, only for a one-off (leave `frequency`/`day_of_week`/`week_of_month`/`interval_weeks`/`anchor_date` blank when this is set) |
...
A one-off show would instead leave `frequency`/`day_of_week`/`week_of_month`/`interval_weeks`/`anchor_date`/`sign_up_method`/`sign_up_url`/`sign_up_other_note`/`sign_up_opens_at`/`cost_to_perform` blank and fill in `one_off_date`, `ticket_price`, `ticket_url`.
