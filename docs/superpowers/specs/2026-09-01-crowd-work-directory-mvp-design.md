...
> **Update (2026-09-09):** The admin "Sources management" page (Key Pages / Components below) is also scoped out of the MVP. The source-check agent is a manually-triggered skill, not yet battle-tested across a variety of venue sources, so letting moderators freely add sources on top of an unproven extraction pipeline is more risk than the MVP needs — sources continue to be registered directly via Supabase Studio, as already noted in [2026-09-05-source-check-agent-design.md](2026-09-05-source-check-agent-design.md). This doesn't block getting new mics/shows listed: moderators and comics can already add them directly via the existing submission and direct-add flows. See [future-considerations.md](../../../notes/future-considerations.md).
