...
- Create: `tests/lib/recurrence.test.ts`
...
- Produces:
  ...
  - Types `RecurrenceRule`, `OccurrenceException`, `Listing` (a union of `RecurringListing` and `OneOffListing`), `Occurrence` — all exported from `src/lib/recurrence.ts`, consumed by `src/lib/listings.ts` (Task 7) and the pages in Tasks 8-9.
...
import { resolveOccurrences } from '../../src/lib/recurrence';
...
- Consumes: `getPublishedListings`, `getExceptionsForListings`, `getAreas`, `toRecurrenceListing` from `src/lib/listings.ts` (Task 7); `resolveOccurrences` from `src/lib/recurrence.ts` (Task 6)
- Produces: the `/` route
